# УСТАНОВКА БИБЛИОТЕК
# pip install aiogram==3.2.0 supabase==1.0.3 python-dotenv==1.0.0 apscheduler==3.10.1 nest-asyncio
...
def main_menu(lang: str):
    ...
    builder.row(InlineKeyboardButton(text=get_text('register', lang), callback_data='register'))
    builder.row(InlineKeyboardButton(text=get_text('horses', lang), callback_data='horses'))
    builder.row(InlineKeyboardButton(text=get_text('schedule', lang), callback_data='schedule'))
    builder.row(InlineKeyboardButton(text=get_text('book', lang), callback_data='book'))
    builder.row(InlineKeyboardButton(text=get_text('my_bookings', lang), callback_data='my_bookings'))
    builder.row(InlineKeyboardButton(text=get_text('change_lang', lang), callback_data='change_lang'))
    return builder.as_markup()

def language_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="🇷🇺 Русский", callback_data='set_lang_ru'),
...
async def cmd_start(message: types.Message):
    ...
    if not user.data:
        await message.answer("Сначала зарегистрируйтесь!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Начинающий", callback_data='level_beginner'),
    ...
    await message.answer(get_text('choose_level', lang), reply_markup=builder.as_markup())
...
@dp.callback_query(F.data == 'horses')
...
async def book_lesson_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    for l in lessons.data:
        ...
        builder.row(InlineKeyboardButton(text=btn_text, callback_data=f"book_{l['id']}"))
    builder.row(InlineKeyboardButton(text="Отмена", callback_data='cancel'))
    await callback.message.answer("Выберите занятие:", reply_markup=builder.as_markup())
    await state.set_state(BookingLesson.select_lesson)
    await callback.answer()

@dp.callback_query(BookingLesson.select_lesson, F.data.startswith('book_'))
async def confirm_booking(callback: types.CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_{lesson_id}"),
                InlineKeyboardButton(text="❌ Отмена", callback_data='cancel'))
    await callback.message.answer(text, reply_markup=builder.as_markup())
    await state.set_state(BookingLesson.confirm)
    await callback.answer()

@dp.callback_query(BookingLesson.confirm, F.data.startswith('confirm_'))
...
async def admin_panel(message: types.Message):
    ...
    lang = 'ru'
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=get_text('add_horse', lang), callback_data='admin_add_horse'))
    builder.row(InlineKeyboardButton(text=get_text('add_lesson', lang), callback_data='admin_add_lesson'))
    builder.row(InlineKeyboardButton(text=get_text('view_bookings', lang), callback_data='admin_view_bookings'))
    await message.answer(get_text('admin_menu', lang), reply_markup=builder.as_markup())

@dp.callback_query(F.data == 'admin_add_horse')
...
@dp.message(AdminAddHorse.desc_ru)
...
@dp.message(AdminAddHorse.desc_kz)
...
@dp.message(AdminAddHorse.desc_en)
...
@dp.message(AdminAddHorse.photo)
...
@dp.message(AdminAddLesson.date)
...
@dp.message(AdminAddLesson.time)
async def admin_lesson_time(message: types.Message, state: FSMContext):
    await state.update_data(time=message.text)
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=get_text('training', 'ru'), callback_data='type_training'),
                InlineKeyboardButton(text=get_text('walk', 'ru'), callback_data='type_walk'))
    await message.answer(get_text('send_lesson_type', 'ru'), reply_markup=builder.as_markup())
    await state.set_state(AdminAddLesson.type)

@dp.callback_query(AdminAddLesson.type, F.data.startswith('type_'))
...
@dp.message(AdminAddLesson.max_participants)
async def admin_lesson_max(message: types.Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    for horse in horses.data:
        builder.row(InlineKeyboardButton(text=horse['name'], callback_data=f"horse_{horse['id']}"))
    await message.answer(get_text('send_lesson_horse', 'ru'), reply_markup=builder.as_markup())
    await state.set_state(AdminAddLesson.horse)

@dp.callback_query(AdminAddLesson.horse, F.data.startswith('horse_'))
...
@dp.message(AdminAddLesson.instructor)
